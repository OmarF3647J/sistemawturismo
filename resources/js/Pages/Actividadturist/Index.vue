...
const v = ref({idacttur: '',nomacttur: '', guiasturist: []
...
const openModalDelete = (actividad) => {
    showModalDelete.value = true;
    v.value = actividad;
    v.value.idacttur = actividad.idacttur;
    v.value.nomacttur = actividad.nomacttur;
...
const deleteActividad = () => {
    form.delete(route('actividadturist.destroy', v.value.idacttur), {
        onSuccess: () => {
...
<template>
    <Head title="Actividades Turísticas" />
    ...
    <AuthenticatedLayout>
                ...
                <table class="w-full whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3">Nombre de la Actividad</th>
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-100">
                        <tr v-for="actividad in actividadturist" :key="actividad.actividadturist"
                            class="text-gray-700">
                            <td class="px-4 py-3 text-sm">{{ actividad.idacttur }}</td>
                            <td class="px-4 py-3 text-sm">{{ actividad.nomacttur}}</td>
                            <td class="px-4 py-3 text-sm">
                                <SecondaryButton @click="openModalView(actividad)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
                            ...
                            <td class="px-4 py-3 text-sm">
                                <DangerButton @click="openModalDelete(actividad)">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
        ...
        <Modal :show ="showModalView" @close="closeModalView">
            <div class="p-6">
                <p>ID de la actividad: <span class="text-lg font-medium text-gray-900">{{ v.idacttur }}</span></p>
                <p>Nombre de la actividad: <span class="text-lg font-medium text-gray-900">{{ v.nomacttur }}</span></p>
        ...
        <Modal :show ="showModalDelete" @close="closeModalDelete">
            <div class="p-6">
                <p class="text-2xl text-gray-500">
                    Estas seguro de eliminar la actividad turística: 
                    <span class="text-2xl font-medium text-gray-900">{{ v.nomacttur }}</span> ?</p>
                    ...
                    <PrimaryButton @click = "deleteActividad">
                        Si, Eliminar
                    </PrimaryButton>
